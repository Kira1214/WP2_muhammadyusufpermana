		...
		<div class="col-lg-12"> 
			<?php if(validation_errors()){?> 
				...
				<table class="table table-hover"> 
					...
					<tbody> 
						<?php $a = 1; foreach ($kuliner as $k) { ?> 
						<tr> 
							<th scope="row"><?= $a++; ?></th> 
							<td><?= $k['nama_kuliner']; ?></td>
							<td><?= $k['keterangan']; ?></td>  
							<td>Rp<?= $k['harga']; ?></td> 
							<td> <picture><source srcset="" type="image/svg+xml"> <img src="<?= base_url('assets/img/upload/') . $k['gambar'];?>" class="img-fluid img-thumbnail" alt="..."> </picture></td> 
							<td><a href="<?= base_url('kuliner/ubahkuliner/').$k['id_kuliner'];?>" class="badge badge-info"><i class="fas fa-edit"></i> Ubah</a> 
								<a href="<?= base_url('kuliner/hapuskuliner/').$k['id_kuliner'];?>" onclick="return confirm('Kamu yakin akan menghapus <?= $judul.' '.$k['nama_kuliner'];?> ?');" class="badge badge-danger"><i class="fas fa-trash"></i> Hapus</a> </td> 
						</tr> 
						<?php } ?> 
					</tbody> 
				</table> 
			...
			<form action="<?= base_url('kuliner'); ?>" method="post" enctype="multipart/form-data"> 
				<div class="modal-body"> 
					...
					<div class="form-group">
						...
						<div class="form-group"> 
							<input type="text" class="form-control form-control-user" id="harga" name="harga" placeholder="Masukkan Harga"> </div> 
						<div class="form-group"> 
